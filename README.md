...
```bash
# 通常 (バックエンド自動検出 — 両方あれば storcli を優先)
./megatui.sh
...
# sudo なし (root で起動、または passwordless 不可な環境)
./megatui.sh --no-sudo
...
├── megatui/
│   ├── __main__.py           # CLI エントリポイント (--backend / --fixtures)
│   ├── runner.py             # 汎用サブプロセスラッパ (sudo / fixture replay)
