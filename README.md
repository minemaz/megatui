...
- **39 アクション** (PD 14 / LD 14 / Adapter 11) を 4 段階の危険度でラベリング
  - `safe` … 状態問い合わせ系
...
- LD の **新規作成 (`-CfgLdAdd`)** は本ツールから発行できない (アレイ構成は MegaCli の引数仕様が複雑なため意図的に省略)。コマンドラインから `MegaCli64 -CfgLdAdd ... ` を直接使うこと。
